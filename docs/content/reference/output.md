...
describes what users get. Pick a format with `-o`, or let uniprot choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
uniprot <command> -o table   # aligned columns for reading
uniprot <command> -o jsonl   # one JSON object per line, for piping
uniprot <command> -o json    # a single JSON array
uniprot <command> -o csv     # spreadsheet friendly
uniprot <command> -o tsv     # tab-separated
uniprot <command> -o url     # just the URL column
uniprot <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
uniprot <command> --fields id,title,url
```
...
```bash
uniprot <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
uniprot <command>            # a table, because this is a terminal
uniprot <command> | wc -l    # JSONL, because this is a pipe
```
