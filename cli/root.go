// Package cli assembles the uniprot command tree from the uniprot
...
// NewApp assembles the kit application from the uniprot domain. The
...
//
// To add a command, declare it in uniprot/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := uniprot.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(uniprot.Domain{}).Register(app)
